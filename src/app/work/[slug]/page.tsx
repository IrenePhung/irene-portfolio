...
> = {
	renopilot: {
		...
		testing: ["Vitest: unit + integration tests for core logic and API behaviour.", "Playwright: E2E smoke tests for critical user flows."],
	...
	localshop: {
		...
		overview: "A full-stack e-commerce prototype. I built authentication, checkout flow, and payment integration with clean architecture.",
		role: ["Implemented JWT authentication and protected routes.", "Built cart/checkout flow and order lifecycle.", "Integrated PayPal end-to-end."],
...
function ActionLink({ label, href }: { label: string; href: string }) {
	...
	if (isInternal) {
		return (
			<Link href={href} className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm hover:bg-white/10">
				{label}
			</Link>
		);
	}
			...
			<div className="mx-auto max-w-4xl px-6 py-14 space-y-10">
				...
				<header className="glass rounded-[32px] p-8">
					...
					<div className="mt-6 flex flex-wrap gap-2">
						{data.links.map((l) => (
							<ActionLink key={l.label} label={l.label} href={l.href} />
						))}
					</div>
				</header>
